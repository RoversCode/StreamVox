...
非永久授权会使用本地在线 token 缓存，并在 token 到期后的下一次合成前自动回服务器复核；永久授权保持长期 token 行为。
...
检查 `license_key` 是否有效，授权服务是否可达，或者是否存在本地有效 token。非永久授权 token 到期后需要回服务器复核；复核失败时会进入 trial 模式。
